...
# Dataset configurations - MODIFIED to include single_speaker flag
DATASETS = [
    {
        ...
        "single_speaker": False  # Multi-speaker dataset - DO NOT concatenate
    },
    {
        ...
        "single_speaker": True  # Single speaker - OK to concatenate
    },
    {
    ...
    for category in categories:
        ...
        for wav_file in tqdm(wav_files, desc=f"{dataset_name}-{category}"):
            try:
                ...
                # Store metadata for this sample - EXCLUDE from concatenation
                dataset_metadata.append({
                    'index': len(csv_data) - 1,
                    'dataset': dataset_name,
                    'single_speaker': False  # Additional datasets are NOT used for concatenation
                })
...
def group_samples_by_speaker_selected_only(csv_data, durations, dataset_metadata):
    """Group samples by speaker - ONLY for selected single-speaker datasets"""
    speaker_groups = defaultdict(list)
    
    for i, (entry, duration, metadata) in enumerate(zip(csv_data, durations, dataset_metadata)):
        # ONLY process samples from selected single-speaker datasets
        if not metadata.get('single_speaker', False):
            continue
        ...
        # Create speaker groups based on dataset source
        if dataset == 'Hindi-F':
            speaker_id = "hindi_f_speaker"
        elif 'skywalker290/Hindi_TTS_M' in str(dataset):
            speaker_id = "skywalker_male_speaker"
        elif 'himanshu23099/singe_speaker_hindi_audio' in str(dataset):
            speaker_id = "himanshu_single_speaker"
        elif 'ujs/hinglish' in str(dataset):
            speaker_id = "hinglish_speaker"
        else:
            # Skip other datasets (including additional female/male)
            continue
...
def create_concatenated_samples_hours_limited_selected_only(speaker_groups, csv_data, durations, global_idx, wavs_dir, 
                                                           target_total_hours=210, max_duration_minutes=2):
    """Create concatenated samples ONLY from selected single-speaker datasets"""
    print(f"\n{'='*60}")
    print("CREATING CONCATENATED SAMPLES (SELECTED SINGLE-SPEAKER ONLY)")
    print(f"{'='*60}")
    
    # Calculate current total hours
    current_total_hours = sum(durations) / 3600
    print(f"Current total duration: {current_total_hours:.2f} hours")
    print(f"Target total duration: {target_total_hours:.2f} hours")
    
    # Calculate how many hours we can add
    available_hours = target_total_hours - current_total_hours
    if available_hours <= 0:
        print(f"No concatenation needed - already at or above target!")
        return [], [], global_idx
    
    print(f"Available hours for concatenation: {available_hours:.2f} hours")
    
    print(f"Processing {len(speaker_groups)} selected single-speaker groups for concatenation:")
    for speaker_id, samples in speaker_groups.items():
        print(f"  {speaker_id}: {len(samples)} samples")
    
    # Estimate average concatenated sample duration (aim for 60-90 seconds)
    avg_concat_duration = 75  # seconds
    target_concat_samples = int((available_hours * 3600) / avg_concat_duration)
    
    print(f"Target concatenated samples: {target_concat_samples}")
    print(f"Average concatenated duration: {avg_concat_duration} seconds")
    ...
    for speaker_id, samples in speaker_groups.items():
        ...
        # Filter samples that are suitable for concatenation
        suitable_samples = [s for s in samples if s['duration'] <= 20.0]  # Max 20 seconds per clip
        
        if len(suitable_samples) < 2:
            continue
            
        # Calculate how many concatenated samples to create for this speaker
        speaker_target = max(1, int(target_concat_samples * len(suitable_samples) / len(csv_data)))
        speaker_created = 0
        ...
        for _ in range(speaker_target * 5):  # Try more times to account for failures
            if speaker_created >= speaker_target:
                break
                
            # Randomly select 4-8 samples to concatenate for longer sequences
            num_to_concat = random.randint(4, min(8, len(suitable_samples)))
            ...
            try:
                ...
                silence_gap = np.zeros(int(sample_rate * 0.3))  # 0.3 second silence
                ...
                if final_duration < 30.0 or final_duration > max_duration_seconds:
                    continue
                ...
                current_concat_hours = sum(concatenated_durations) / 3600
                if current_concat_hours >= available_hours * 0.95:  # Stop at 95% of target
                    print(f"  Reached target hours limit for {speaker_id}")
                    break
    ...
    final_concat_hours = sum(concatenated_durations) / 3600
    print(f"\nTotal concatenated samples created: {created_samples}")
    print(f"Total concatenated duration: {final_concat_hours:.2f} hours")
    print(f"NOTE: Only selected single-speaker datasets were used for concatenation:")
    print(f"  - skywalker290/Hindi_TTS_M")
    print(f"  - himanshu23099/singe_speaker_hindi_audio") 
    print(f"  - ujs/hinglish")
    print(f"  - Hindi-F (local dataset)")
    print(f"Additional female/male datasets and multi-speaker datasets are included but NOT concatenated.")
    
    return concatenated_data, concatenated_durations, global_idx

def main():
    # Configuration
    output_dir = "Combined_Hindi_TTS_Hours_Limited_Selected_Speakers"  # Updated output directory name
    ...
    target_total_hours = 210  # Target total hours (200-215 range)
    ...
    print(f"Starting multi-dataset processing with SELECTED SINGLE-SPEAKER CONCATENATION...")
    print(f"Target total duration: {target_total_hours} hours")
    print(f"NOTE: Only specific single-speaker datasets will be used for concatenation")
    ...
    print(f"NOTE: Female dataset will NOT be used for concatenation")
    ...
    print(f"NOTE: Male dataset will NOT be used for concatenation")
    ...
    for dataset_config in DATASETS:
        ...
        if config.get("single_speaker", False):
            print(f"  -> Single-speaker dataset: Will be used for concatenation")
        else:
            print(f"  -> Multi-speaker dataset: Will NOT be used for concatenation")
        total_valid_samples += dataset_valid_samples
    
    # Analyze duration distribution
    analyze_duration_distribution(durations)
    
    # Group samples by speaker - ONLY selected single-speaker datasets
    print(f"\n{'='*60}")
    print("GROUPING SELECTED SINGLE-SPEAKER SAMPLES BY SPEAKER")
    print(f"{'='*60}")
    speaker_groups = group_samples_by_speaker_selected_only(csv_data, durations, dataset_metadata)
    
    print(f"Found {len(speaker_groups)} selected single-speaker groups:")
    for speaker_id, samples in speaker_groups.items():
        avg_duration = np.mean([s['duration'] for s in samples])
        print(f"  {speaker_id}: {len(samples)} samples, avg duration: {avg_duration:.2f}s")
    
    # Create concatenated samples - ONLY from selected single-speaker datasets
    concatenated_data, concatenated_durations, global_idx = create_concatenated_samples_hours_limited_selected_only(
        speaker_groups, csv_data, durations, global_idx, wavs_dir, 
        target_total_hours=target_total_hours, max_duration_minutes=2
    )
    ...
    print(f"\nIMPORTANT: This version only concatenates samples from these specific datasets:")
    print(f"  - skywalker290/Hindi_TTS_M")
    print(f"  - himanshu23099/singe_speaker_hindi_audio") 
    print(f"  - ujs/hinglish")
    print(f"  - Hindi-F (local dataset)")
    print(f"\nDatasets included but NOT used for concatenation:")
    print(f"  - Additional female/male datasets (local)")
    print(f"  - SPRINGLab/IndicTTS-Hindi (multi-speaker)")
    print(f"  - SPRINGLab/IndicTTS-English (multi-speaker)")
    print(f"{'='*60}")
